            ...
            op_heads_store::resolve_op_heads(
                ...
                async |op_heads| {
                    ...
                    for other_op_head in op_heads.into_iter().skip(1) {
                        tx.merge_operation(other_op_head).block_on()?;
                    ...
                    Ok(tx
                        .write("reconcile divergent operations").block_on()?
                        .leave_unpublished()
                        .operation()
                        .clone())
                },
...
impl WorkspaceCommandHelper {
    ...
    fn import_git_head(&mut self, ui: &Ui) -> Result<(), CommandError> {
        ...
        if let Some(new_git_head_id) = new_git_head.as_normal() {
            ...
            let wc_commit = tx
                .repo_mut()
                .check_out(workspace_name, &new_git_head_commit).block_on()?;
...
",
    ...
    fn snapshot_working_copy(
        ...
        let (repo, wc_commit) =
            match WorkingCopyFreshness::check_stale(locked_ws.locked_wc(), &wc_commit, &repo).block_on() {
    ...
    fn finish_transaction(
        ...
        for (name, wc_commit_id) in &tx.repo().view().wc_commit_ids().clone() {
                ...
                .is_some()
            {
                ...
                tx.repo_mut().check_out(name.clone(), &wc_commit).block_on()?;
                writeln!(
                    ui.warning_default(),
