...
pub fn cmd_debug_tree(
    ...
    let matcher = workspace_command
        ...
        .to_matcher();
    let mut entries: Pin<
        Box<dyn Stream<Item = (RepoPathBuf, Result<Merge<Option<TreeValue>>, _>)>>,
    > = if let Some(tree_id_hex) = &args.id {
        ...
        let tree = store.get_tree_async(dir, &tree_id).block_on()?;
        ...
        Box::pin(tree.entries_matching(matcher.as_ref()))
    };
