...
pub fn cmd_op_abandon(
    ...
    let current_head_ops =
        op_walk::get_current_head_ops(op_store, op_heads_store.as_ref()).block_on()?;
    let resolve_op =
        |op_str| op_walk::resolve_op_at(op_store, &current_head_ops, op_str).block_on();
    ...
    if !command.global_args().ignore_working_copy {
        ...
        if let Some((_, new_id)) = reparented_head_ops().find(|(old, _)| old.id() == old_op_id) {
            locked_ws.finish(new_id.clone()).block_on()?;
        } else {
            writeln!(
                ui.warning_default(),
