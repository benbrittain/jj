...
fn test_bad_locking_children(backend: TestRepoBackend) {
    ...
    .unwrap();
    let machine1_repo = machine1_workspace.repo_loader().load_at_head().block_on().unwrap();
    ...
    .unwrap();
    let machine2_repo = machine2_workspace.repo_loader().load_at_head().block_on().unwrap();
    ...
    .unwrap();
    let merged_repo = merged_workspace.repo_loader().load_at_head().block_on().unwrap();
