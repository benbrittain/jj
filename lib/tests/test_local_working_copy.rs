...
fn test_checkout_no_op() {
    ...
    let ws = &mut test_workspace.workspace;
    ws.check_out(repo.op_id().clone(), None, &commit1).block_on().unwrap();
    ...
    let stats = ws.check_out(new_op_id.clone(), None, &commit2).block_on().unwrap();
...
fn test_acl() {
    ...
    ws.check_out(repo.op_id().clone(), None, &commit2).block_on().unwrap();
    assert!(
        !secret_modified_path
            .to_fs_path_unchecked(&workspace_root)
            .is_file()
...
fn test_tree_builder_file_directory_transition() {
    ...
    let mut check_out_tree = |tree_id: &TreeId| {
        ...
        ws.check_out(repo.op_id().clone(), None, &commit).block_on().unwrap();
    };
...
fn test_conflicting_changes_on_disk() {
    ...
    let stats = ws.check_out(repo.op_id().clone(), None, &commit).block_on().unwrap();
    assert_eq!(
        stats,
        CheckoutStats {
            updated_files: 0,
...
fn test_snapshot_special_file() {
    ...
    let (tree, _stats) = locked_ws
        ...
        .block_on()
        .unwrap();
    locked_ws.finish(OperationId::from_hex("abc123")).block_on().unwrap();
...
fn test_gitignores_in_ignored_dir() {
    ...
    let ws = &mut test_workspace.workspace;
    ws.check_out(op_id.clone(), None, &commit1).block_on().unwrap();
...
fn test_gitignores_checkout_never_overwrites_ignored() {
    ...
    // working copy.
    let ws = &mut test_workspace.workspace;
    assert!(ws.check_out(repo.op_id().clone(), None, &commit,).block_on().is_ok());
...
fn test_check_out_existing_file_cannot_be_removed() {
    ...
    let result = ws.check_out(repo.op_id().clone(), None, &commit2).block_on();
...
fn test_check_out_existing_file_replaced_with_directory() {
    ...
    let stats = ws.check_out(repo.op_id().clone(), None, &commit2).block_on().unwrap();
    assert_eq!(stats.skipped_files, 1);
...
fn test_check_out_reserved_file_path(file_path_str: &str) {
    ...
    // Checkout should fail.
    let ws = &mut test_workspace.workspace;
    let result = ws.check_out(repo.op_id().clone(), None, &commit1).block_on();
