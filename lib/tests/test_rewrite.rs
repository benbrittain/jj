...
fn test_rebase_descendants_basic_bookmark_update() {
    ...
    let mut tx = repo.start_transaction();
    let commit_b2 = tx.repo_mut().rewrite_commit(&commit_b).block_on().write().block_on().unwrap();
...
fn test_rebase_descendants_update_bookmarks_after_divergent_rewrite() {
    ...
    let commit_b41 = tx.repo_mut().rewrite_commit(&commit_b4).block_on().write().block_on().unwrap();
    let commit_b42 = tx
        .repo_mut()
...
fn test_rebase_descendants_rewrite_updates_bookmark_conflict() {
    ...
    let mut tx = repo.start_transaction();
    let commit_a2 = tx.repo_mut().rewrite_commit(&commit_a).block_on().write().block_on().unwrap();
    // Different description so they're not the same commit
    let commit_a3 = tx
        .repo_mut()
...
fn test_rebase_abandoning_empty() {
    ...
    rebase_commit_with_options(rewriter, &rebase_options).block_on().unwrap();
