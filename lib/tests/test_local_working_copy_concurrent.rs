...
fn test_concurrent_checkout() {
    ...
    ws1.check_out(repo.op_id().clone(), None, &commit1).block_on().unwrap();
    ...
    assert_matches!(
        ws1.check_out(repo.op_id().clone(), Some(&tree1), &commit3,).block_on(),
    ...
    thread::scope(|s| {
        for tree in &trees {
            ...
            s.spawn(move || {
                ...
                // The operation ID is not correct, but that doesn't matter for this test
                let stats = workspace.check_out(op_id, None, &commit).block_on().unwrap();
...
fn test_racy_checkout() {
    ...
    for _ in 0..100 {
        ...
        ws.check_out(op_id.clone(), None, &commit).block_on().unwrap();
        assert_eq!(
