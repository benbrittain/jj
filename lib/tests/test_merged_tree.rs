...
fn test_merged_tree_builder_resolves_conflict() {
    ...
    let base_tree = MergedTree::unlabeled(
        store.clone(),
...
fn test_path_value_and_entries() {
    ...
    let merged_tree = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
            vec![tree1.id().clone()],
    ...
    assert_eq!(
        merged_tree
            .path_value_async(RepoPath::root())
            .block_on()
            .unwrap(),
        Merge::from_removes_adds(
    ...
    assert_eq!(
        merged_tree
            .path_value_async(resolved_file_path)
            .block_on()
            .unwrap(),
    ...
    assert_eq!(
        merged_tree
            .path_value_async(resolved_dir_path)
            .block_on()
            .unwrap(),
    ...
    assert_eq!(
        merged_tree
            .path_value_async(modify_delete_path)
            .block_on()
            .unwrap(),
        Merge::from_removes_adds(
    ...
    assert_eq!(
        merged_tree
            .path_value_async(file_dir_conflict_path)
            .block_on()
            .unwrap(),
        Merge::from_removes_adds(
    ...
    assert_eq!(
        merged_tree
            .path_value_async(file_dir_conflict_sub_path)
            .block_on()
            .unwrap(),
        Merge::absent(),
    );
    ...
    .map(|&path| {
        (
            path.to_owned(),
            merged_tree.path_value_async(path).block_on().unwrap(),
        )
    })
    .collect_vec();
...
fn test_diff_copy_tracing_file_and_dir() {
    ...
    assert_eq!(
        ...
        (
            ...
            Diff::new(
                before
                    .path_value_async(repo_path("b/file"))
                    .block_on()
                    .unwrap(),
                after
                    .path_value_async(repo_path("a/file"))
                    .block_on()
                    .unwrap(),
            ),
        )
    ...
    assert_eq!(
        ...
        (
            ...
            Diff::new(
                before.path_value_async(repo_path("a")).block_on().unwrap(),
                after.path_value_async(repo_path("b")).block_on().unwrap(),
            ),
        )
    );
    assert_eq!(
        ...
        (
            ...
            Diff::new(
                before.path_value_async(repo_path("c")).block_on().unwrap(),
                after
                    .path_value_async(repo_path("c/file"))
                    .block_on()
                    .unwrap(),
            ),
        )
...
fn test_diff_conflicted() {
    ...
    let left_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
    ...
    let right_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
    ...
    let expected_diff = [path2, path3, path4]
        ...
        .map(|&path| {
            (
                path.to_owned(),
                (
                    left_merged.path_value_async(path).block_on().unwrap(),
                    right_merged.path_value_async(path).block_on().unwrap(),
                ),
            )
        })
        .collect_vec();
...
fn test_diff_dir_file() {
    ...
    let left_value = |path: &RepoPath| left_merged.path_value_async(path).block_on().unwrap();
    let right_value = |path: &RepoPath| right_merged.path_value_async(path).block_on().unwrap();
...
fn test_merge_partial_resolution() {
    ...
    let side1_merged = MergedTree::resolved(repo.store().clone(), side1.id().clone());
    let side2_merged = MergedTree::resolved(repo.store().clone(), side2.id().clone());
    let expected_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
...
fn test_merge_simplify_only() {
    ...
    let base1_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
            vec![tree1.id().clone()],
            vec![tree2.id().clone(), tree3.id().clone()],
    ...
    let side1_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
            vec![tree1.id().clone()],
    ...
    let side2_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
            vec![tree4.id().clone()],
...
{suffix}"#
    ...
    let parent_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
    ...
    let child1_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
...
fn test_merge_simplify_file_conflict_with_absent() {
    ...
    let child2_merged = MergedTree::unlabeled(
        repo.store().clone(),
        Merge::from_removes_adds(
