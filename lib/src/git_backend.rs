...
pub enum GitBackendInitError {
    ...
    #[error("Failed to open git backend due to missing config")]
    MissingGitConfig(#[source] ConfigGetError),
    #[error(transparent)]
    Path(PathError),
}
...
impl GitBackend {
    ...
    pub fn init_internal(
        ...
        let change_id_setting = settings
            .git_settings()
            .map_err(GitBackendInitError::MissingGitConfig)?
            .change_id;
...
fn commit_from_git_without_root_parent(
    ...
    let change_id = commit
        .extra_headers()
        .find("change-id")
        .and_then(|reverse_hex| to_forward_hex(reverse_hex.to_string().as_str()))
...
impl Backend for GitBackend {
    ...
    async fn write_commit(
        ...
        let id = loop {
            ...
            // NOTE: I don't think this is needed in the world where we write
            // change-id as a git header since the git id will be different.
...
mod tests {
    ...
    fn round_trip_change_id_via_git_header() {
        ...
        let original_change_id = ChangeId::from_hex("c64ee0b6e16777fe53991f9281a6cd25");
        let commit = gix::objs::Commit {
            ...
            encoding: None,
            message: "git commit message".into(),
            extra_headers: Vec::from([(
                BString::new(CHANGE_ID_COMMIT_HEADER.to_vec()),
                BString::new(original_change_id.reverse_hex().into()),
            )]),
        };
